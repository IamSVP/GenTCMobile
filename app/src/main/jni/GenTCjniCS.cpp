...
void RendererCS::initializeMPTCTexture() {
    ...
    prev_dxt = (PhysicalDXTBlock*)malloc(sizeof(PhysicalDXTBlock));
    curr_dxt = (PhysicalDXTBlock*)malloc(sizeof(PhysicalDXTBlock));
    ptr_decode_info = (MPTCDecodeInfo*)malloc(sizeof(MPTCDecodeInfo));
    ptr_decode_info->is_start = true;
    m_mptc_stream.open(m_mptc_file_path.c_str(), std::ios::binary);
    ALOGE("Called load Texture Data,,,,,");

    //ptr_buffer_struct = (BufferStruct*)malloc(sizeof(BufferStruct));
    //num_blocks = vImageHeight/4 * vImageWidth/4;
    //ALOGE("Here tooo.....");
    //InitBufferedDecode(5, ptr_buffer_struct, m_mptc_stream, num_blocks);

}

void RendererCS::loadTextureDataMPTC() {
    ...
    GetFrame(m_mptc_stream, prev_dxt, curr_dxt, ptr_decode_info);
    //GetBufferedFrame(ptr_buffer_struct, curr_dxt, m_mptc_stream);
    std::chrono::high_resolution_clock::time_point CPUDecode_End = std::chrono::high_resolution_clock::now();
    ALOGE("curr_dxt value:%d", curr_dxt);
